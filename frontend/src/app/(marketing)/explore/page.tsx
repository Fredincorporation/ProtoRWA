 ...
 *
 * Implements the PRD spec from screen 07: "Grid or list of all live projects.
 * Filters: category, funding status (open / funded / completed), milestone
 * stage, funding goal range, return multiple. Sort by newest, most funded,
 * ending soon. Search bar."
 *
 * Client-side filter state is appropriate here: the dataset is small and the
 * interaction must feel instant. The filter functions are pure and exported, so
 * moving to server-side filtering later is a local change.
 */
export default function ExplorePage() {
  const [filters, setFilters] = React.useState<ExploreFilters>(defaultFilters);

  const results = React.useMemo(() => applyFilters(mockProjects, filters), [filters]);
